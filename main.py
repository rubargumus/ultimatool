...
"""
			...
			print(banner)	
			print("-"*80)
			print("Ornek : nuke-force.net Yada www.nuke-force.net")
			print("-"*80)
			print("Not: https,http Kullanmayın Sonunda / Olmasın ")
			print("-"*80)
			url = input("URL? : ")
			print("-"*80)
			root = input("Bu Website https kullanıyormu ? y/n  : ")
			print("-"*80)
			if root == "y" or "Y" :
				new = "https://"+url+"/"
			elif root == "n" or "N":
				new = "http://"	+ url+"/"	 
			liste=['admin/','administrator/','login.php','administration/','admin1/','admin2/','admin3/','admin4/','admin5/','moderator/','webadmin/','adminarea/','bb-admin/','adminLogin/','admin_area/','panel-administracion/','instadmin/',
...
'adm/index.brf','adm.brf','affiliate.brf','adm_auth.brf','memberadmin.brf','administratorlogin.brf','cpanel','cpanel.php','cpanel.html',]
			...
			for i in liste :
				...
				if response.status_code == 404 :
					print("Bulunamadı {}".format(req))
				elif response.status_code == 403 :
					print("HATA (MUHTEMEL GÜVENLİK DUVARI) {}".format(response.status_code))
				else :
					print("Bulundu {}".format(req))	
			...
			if bulunanlar == [] :
				print(banner)
				print("-"*80)
				print("Admin Paneli Bulunamadı")
			else :
				print(banner)
				print("-"*80)
				print("-"*8 ,"BULUNANLAR","-"*8)
				for i in bulunanlar :
					print("Sonuç : {}".format(i))
			break
		elif islemno == 2:
...
<center>
<br><br><br><br>
<img src="https://www.freepnglogos.com/uploads/turk-bayragi-png/turkish-flag-turk-bayragi-crescent-moon-and-star-pictures-3.png" >
<br><br>
<h3>U RE HACKT !!!</h3>
<font size="20" color="red">Hacked By {} </font>

</center>
</body>
</html>""" .format(css,uname)
...
""")
			...
			try:
				os.system("clear")
				print("dont use http(s)  example: xxx.com ")
				siten = input("Site? ")
				os.system("nmap -sS -sV {}".format(siten))
				os.system("nikto -host  {}".format(siten))
				break
			except: 
				print("Hata İle Karşılaşıldı Lütfen Github Sayfasındaki Kurulum Bölümünü Okuyunuz")
		elif islemno == 99:
			exit()
		else:
			print("Bu İşlem Numarasına Sahip ' {} '  Bir İşlem Bulunmamaktadır \n ".format(islemno))
			time.sleep(3)
			break
...
if os.name=="posix":
	os.system("clear")
	print ("İşletim Sisteminiz Linux Olarak Tespit Edildi")
	time.sleep(2)
	os.system("clear")
	print(banner)
	print(main)
	isn = int(input("İşlem No : "))
	nfx(isn)


else:
	print("Lütfen Linux Kullanın sizin işletim sisteminiz {}".format(os.name))
	if os.name == "nt":
		print("İşletim Sisteminiz Windows Olarak Tespit Edildi")
	time.sleep(4)
	exit()